...
it kind of makes it hard to decide when a function is dead,
...
Some of the benchmarks were claimed 'incorrect' by brench but I spot checked running them manually with brili and they seemed fine.
The problematic benchmarks for brench seemed to be ones with args/turnt attributes.
** lesson 4
dataflow is cool!
...
** lesson 7: LLVM
follow [[https://www.cs.cornell.edu/~asampson/blog/llvm.html][blog]]
SSA form IR with a nice API for manipulating it

uniform IR across passes

[[https://llvm.org/docs/WritingAnLLVMPass.html][canonical pass writing reference]]
** Lesson 8: Loop Optimizations
loop unswitching lifts loop invariant conditionals out of the loop + duplicates the loop for each branch

loop fusion merges loops to inline composition instead of using intermediate structures

inductive variable weakening coarsens the granularity of loop var updates (++ to +=2, etc.)

** lesson 9: Inter-procedural Optimization

call graph

open vs closed (nothing else will interact with these functions) world assumption

inlining, devirtualizing function calls can be good sometimes
** lesson 10: pointer/alias analysis
the potential for aliasing makes it very challenging to do things like autovectorization

try to decide yes/no/maybe aliased
